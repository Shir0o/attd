...
- **Hub "Taken" State After All-Present Confirm**: A session confirmed with "Start with all present" (no exceptions toggled) is no longer silently discarded on return to the hub, so the event correctly shows its taken/Marked state. Keep-vs-discard is now decided by the action taken — cancelling (X / system back) discards the preseeded session, confirming (Done) keeps it — via a `PopScope` on the deck gated by a `deleteOnCancel` flag (only set for freshly created sessions, so resuming an existing incomplete session and backing out preserves its records), instead of guessing from whether records still bore the preseed author (`System (Preseed)`), which misclassified an untouched all-present confirmation as an abandoned session.
