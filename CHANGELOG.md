...
### Backfill from prior history (v1.0.1+2 → v1.3.2+24)

Per-version notes migrated verbatim from the legacy `release.md` when that
file was removed in favor of this release-please-owned CHANGELOG (see #146).
These versions predate the release-please pipeline and never got separate
GitHub Releases; their history is preserved here in one place.
